...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['CZBET_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['CZBET_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Czbet API.
 */
export class Czbet {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['CZBET_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('CZBET_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.CzbetError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Czbet client with an apiKey option, like new Czbet({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Czbet.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('CZBET_LOG'), "process.env['CZBET_LOG']", this) ??
      defaultLogLevel;
  ...
  static CzbetError = Errors.CzbetError;
